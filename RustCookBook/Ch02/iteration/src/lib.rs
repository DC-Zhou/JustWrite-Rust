...
pub fn count_files(path: &String) -> usize{
    path.len()
}
...
mod tests {
    ...
    fn data_transformations(){
        ...
        assert_eq!(
            dir_file_counts,
            vec![
                (&"home/alice".to_string(), 10),
                (&"home/bob".to_string(), 8),
                (&"home/carl".to_string(), 9),
                (&"home/debra".to_string(), 10)
            ]
        )
    }
    ...
    fn data_filtering(){
        ...
        let (validation, train) : (Vec<i32>, Vec<i32>) = data.iter().partition(|&_| (rand::random::<f32>() % 1.0) > 0.8);
