...
///
/// Doubles each element in the provided chunks in parallel and returns the result
///
...
fn main() {
    ...
    let results:Vec<i32> = parallel_map(data.clone()).into_iter().flat_map(|thread| thread.join().unwrap()).collect();
